...
-- Type your code below:
select count(DISTINCT id) as num_of_movie ,year
from movie 
...
WITH top_actress
     AS (SELECT b.NAME
         ...
         WHERE  a.category = 'actress'
                AND d.country LIKE '%India%' AND d.languages LIKE "%Hindi%"
         GROUP  BY a.name_id,
                   b.NAME
...
+---------------+-------------------+---------------------+----------------------+-----------------+*/
-- Type your code below:
SELECT name as actress_name, SUM(total_votes) AS total_votes, COUNT(rm.movie_id) as movie_count, Round(Sum(avg_rating*total_votes)/Sum(total_votes),2) AS actress_avg_rating,
